...
class Resource:
    ...
    _symbolicname: str
    name: str = field(init=False, default_factory=lambda: UniqueName(prefix="antisch", length=24), metadata={'rest': 'name'})
...
class LocatedResource(Resource):
    ...
    def __post_init__(self, friendly_name: Optional[str] = None):
        ...
        self.tags['cloudmachine-resource-name'] = self._fname


@dataclass_model
class ResourceGroup(LocatedResource):
    name: str = field(init=False, default_factory=lambda: UniqueName(prefix="antisch", length=24, basestr=CloudMachineName()), metadata={'rest': 'name'})
