...
def azd_env_name(name: str, host: str, label: Optional[str]) -> str:
    return f'cloudmachine-{name}-{"local" if host == "local" else "remote"}' + (f'-{label}' if label else '')


def init_project(root_path: str, name: str, host: str, label: Optional[str]) -> None:
    azure_dir = os.path.join(root_path, ".azure")
    azure_yaml = os.path.join(root_path, "azure.yaml")
    project_name = azd_env_name(name, host, label)
...
class CloudMachineDeployment:
    ...
    def __init__(
        ...
        *,
        ...
        host: Literal['local', 'appservice'] = 'local',
    ) -> None:
        self.name = name
