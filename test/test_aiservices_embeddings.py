...
from azure.cloudmachine.resources.ai.deployment import AIEmbeddings
...
def _get_outputs(suffix="", parent="", rg=None):
    outputs = {
        'resource_id': Output(f"AZURE_AI_EMBEDDINGS_ID{suffix.upper()}", "id", ResourceSymbol(f"embeddings_deployment{suffix}")),
        'name': Output(f"AZURE_AI_EMBEDDINGS_NAME{suffix.upper()}", "name", ResourceSymbol(f"embeddings_deployment{suffix}")),
        'resource_group': Output(f"AZURE_AI_EMBEDDINGS_RESOURCE_GROUP{suffix.upper()}", rg if rg else DefaultResourceGroup().name),
    }
    outputs.update({
        'endpoint': Output(
            f"AZURE_AI_EMBEDDINGS_ENDPOINT{suffix.upper()}",
            Output("", "properties.endpoint", ResourceSymbol(f'aiservices_account{parent}')).format("{}openai/deployments/") + outputs['name'].format() + "/embeddings"
        ),
        'model_name': Output(f"AZURE_AI_EMBEDDINGS_MODEL_NAME{suffix.upper()}", "properties.model.name", ResourceSymbol(f"embeddings_deployment{suffix}")),
        'model_version': Output(f"AZURE_AI_EMBEDDINGS_MODEL_VERSION{suffix.upper()}", "properties.model.version", ResourceSymbol(f"embeddings_deployment{suffix}")),
    })
    return outputs
...
def test_aiservices_embeddings_properties():
    ...
    assert fields['aiservices_account.embeddings_deployment'].resource == "Microsoft.CognitiveServices/accounts/deployments"
    assert fields['aiservices_account.embeddings_deployment'].properties == {'properties': {}, 'parent': ResourceSymbol('aiservices_account')}
    ...
    assert list(fields.keys()) == ['aiservices_account', 'aiservices_account.embeddings_deployment', 'aiservices_account.embeddings_deployment_gpt4o']
    assert fields['aiservices_account.embeddings_deployment_gpt4o'].resource == "Microsoft.CognitiveServices/accounts/deployments"
    assert fields['aiservices_account.embeddings_deployment_gpt4o'].properties == {'name': 'gpt-4o', 'sku': {'capacity': 10}, 'properties': {'model': {'name': 'gpt-4o'}}, 'parent': ResourceSymbol('aiservices_account')}
    ...
    r4 = AIEmbeddings(model="secret", account=AIServices(name='foo', tags={'test': 'value'}, public_network_access='Disabled'))
    assert r4.properties == {'properties': {'model': {'name': 'secret'}}, 'name': 'secret'}
    symbols = r4.__bicep__(fields, parameters=dict(GLOBAL_PARAMS))
    assert list(fields.keys()) == ['aiservices_account', 'aiservices_account.embeddings_deployment', 'aiservices_account.embeddings_deployment_gpt4o', 'aiservices_account_foo', 'aiservices_account_foo.embeddings_deployment_foo_secret']
    assert fields['aiservices_account_foo.embeddings_deployment_foo_secret'].resource == "Microsoft.CognitiveServices/accounts/deployments"
    assert fields['aiservices_account_foo.embeddings_deployment_foo_secret'].properties == {'properties': {'model': {'name': 'secret'}}, 'name': 'secret', 'parent': ResourceSymbol('aiservices_account_foo')}
    assert fields['aiservices_account_foo.embeddings_deployment_foo_secret'].outputs == _get_outputs("_foo_secret", "_foo")
    ...
    assert fields['aiservices_account.embeddings_deployment_foo'].resource == "Microsoft.CognitiveServices/accounts/deployments"
    assert fields['aiservices_account.embeddings_deployment_foo'].properties == {'name': 'foo', 'sku': {'name': param2, 'capacity': param3}, 'properties': {'model': param1}, 'parent': ResourceSymbol('aiservices_account')}
...
def test_aiservices_embeddings_reference():
    ...
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].resource == "Microsoft.CognitiveServices/accounts/deployments"
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].properties == {'name': 'foo', 'parent': ResourceSymbol('aiservices_account_bar')}
    ...
    fields = {}
    symbols = r.__bicep__(fields, parameters=dict(GLOBAL_PARAMS))
    assert list(fields.keys()) == ['resourcegroup_baz', 'aiservices_account_bar', 'aiservices_account_bar.embeddings_deployment_bar_foo']
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].resource == "Microsoft.CognitiveServices/accounts/deployments"
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].properties == {'name': 'foo', 'parent': ResourceSymbol('aiservices_account_bar')}
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].outputs == _get_outputs("_bar_foo", '_bar', 'baz')
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].extensions == {}
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].existing == True
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].version
    assert fields['aiservices_account_bar.embeddings_deployment_bar_foo'].symbol == symbols[0]
    ...
    r = AIEmbeddings.reference(name='foo', account=AIServices.reference(name='bar', resource_group=ResourceGroup.reference(name='baz', subscription=TEST_SUB)))
    assert r.properties == {'name': 'foo'}
...
def test_aiservices_embeddings_app():
    ...
    class TestApp(AzureApp):
        client: EmbeddingsClient = client()
    ...
    class TestApp(AzureApp):
        client: AsyncEmbeddings = client(default=r, api_version="v1.0")
    ...
    class TestApp(AzureApp):
        client: EmbeddingsClient = client(default=TestInfra.ai)
