...
class CloudMachineDeployment:
    ...
    def __init__(
        ...
        *,
        ...
        events: bool = False,
        messaging: Union[bool, ServiceBusNamespace] = False,
        ...
        if storage:
            ...
            if self._storage_resource.tables:
                self.data = resources.get('storage:table').by_name('cloudmachine')
            self.storage = resources.get('storage:blob').by_name('cloudmachine')
        ...
        if messaging:
            if isinstance(messaging, ClientSettings):
                ...
                self.messaging = resources.get('servicebus').by_name('cloudmachine')
                self.messaging.set('default_topic', 'cm_default_topic')
                self.messaging.set('default_subscription', 'cm_default_subscription')
        ...
        if openai:
            if isinstance(openai, ClientSettings):
                ...
                self.openai = resources.get('openai').by_name('cloudmachine')
        ...
        if documentai:
            if isinstance(documentai, ClientSettings):
                ...
                self.documentai = resources.get('documentai').by_name('cloudmachine')
                self.documentai.set('default_model', 'prebuilt-layout')
        self.search = None
        if search:
            if isinstance(search, ClientSettings):
                ...
                self.search = resources.get('search').by_name('cloudmachine')
                self.search.set('document_embedding_index', 'documentembeddingindex')
